...
export interface CliArgs {
  ...
  // Server mode for VSCode extension integration
...
export async function parseArguments(): Promise<CliArgs> {
  const yargsInstance = yargs(hideBin(process.argv))
    ...
    .option('server-mode', {
      type: 'boolean',
      description: 'Run in server mode for VSCode extension integration',
      default: false,
    })
...
export async function loadCliConfig(
  ...
  return new Config({
    ...
    openaiApiUrl: argv.openaiApiUrl,
    openaiOrganization: argv.openaiOrganization,
  });
}
